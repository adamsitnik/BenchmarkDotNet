...
namespace BenchmarkDotNet.Portability
{
    internal static class TypeExtensions
    {
        ...
        internal static IEnumerable<T> GetCustomAttributes<T>(this Type sourceType, Type type, bool inherit)
            where T : Attribute
        {
#if !CORE
            return sourceType.GetCustomAttributes(type, inherit).OfType<T>();
#else
            return sourceType.GetTypeInfo().GetCustomAttributes(type, inherit).OfType<T>();
#endif
        }
